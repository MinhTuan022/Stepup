...
public class ChiTietSanPhamService {
        private final stepup.shoes.repository.HinhAnhChiTietRepository hinhAnhChiTietRepository;
    ...
    public ChiTietSanPhamDTO create(Integer maSanPham, ChiTietSanPhamDTO dto) {
        ...
        if (dto.getHinhAnhs() != null && !dto.getHinhAnhs().isEmpty()) {
            for (stepup.shoes.dto.HinhAnhChiTietDTO imgDto : dto.getHinhAnhs()) {
                stepup.shoes.entity.HinhAnhChiTiet img = stepup.shoes.entity.HinhAnhChiTiet.builder()
                    .chiTietSanPham(saved)
                    .duongDan(imgDto.getDuongDan())
                    .laAnhChinh(imgDto.getLaAnhChinh() != null ? imgDto.getLaAnhChinh() : false)
                    .thuTu(imgDto.getThuTu() != null ? imgDto.getThuTu() : 0)
                    .build();
                hinhAnhChiTietRepository.save(img);
            }
        }
    ...
    public ChiTietSanPhamDTO update(Integer id, ChiTietSanPhamDTO dto) {
        ...
        hinhAnhChiTietRepository.deleteAll(hinhAnhChiTietRepository.findByChiTietSanPham_MaChiTiet(updated.getMaChiTiet()));
        if (dto.getHinhAnhs() != null && !dto.getHinhAnhs().isEmpty()) {
            for (stepup.shoes.dto.HinhAnhChiTietDTO imgDto : dto.getHinhAnhs()) {
                stepup.shoes.entity.HinhAnhChiTiet img = stepup.shoes.entity.HinhAnhChiTiet.builder()
    ...
    private ChiTietSanPhamDTO mapToDTO(ChiTietSanPham chiTiet) {
        List<stepup.shoes.entity.HinhAnhChiTiet> images = hinhAnhChiTietRepository.findByChiTietSanPham_MaChiTietOrderByThuTuAsc(chiTiet.getMaChiTiet());
        List<stepup.shoes.dto.HinhAnhChiTietDTO> imageDTOs = images.stream()
            .map(img -> stepup.shoes.dto.HinhAnhChiTietDTO.builder()
                .maHinhAnh(img.getMaHinhAnh())
                .maChiTiet(img.getChiTietSanPham().getMaChiTiet())
